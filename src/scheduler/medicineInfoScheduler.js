...
async function getMedicineUpdateDeFromDB() {
    ...
    try {
        const [rows] = await connection.execute('SELECT MAX(updateDe) AS updateDe FROM inc.medicineApiData')
...
async function getGrainMedicineChangeDateFromDB() {
    ...
    try {
        const [rows] = await connection.execute('SELECT MAX(CHANGE_DATE) AS CHANGE_DATE FROM inc.grainMedicineApiData')
    ...
    try {
        for (const item of fetchedData) {
            await connection.execute(
                'INSERT INTO inc.medicineApiData (entpName, itemName, itemSeq, efcyQesitm, useMethodQesitm, atpnWarnQesitm, atpnQesitm, intrcQesitm, seQesitm, depositMethodQesitm, openDe, updateDe, itemImage) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)',
...
async function updateGrainMedicineApiData(fetchedData) {
    ...
    try {
        for (const item of fetchedData) {
            await connection.execute(
                'INSERT INTO inc.grainMedicineApiData (ITEM_SEQ, ITEM_NAME, ENTP_SEQ, ENTP_NAME, CHART, ITEM_IMAGE, PRINT_FRONT, PRINT_BACK, DRUG_SHAPE, COLOR_CLASS1, COLOR_CLASS2, LINE_FRONT, LINE_BACK, LENG_LONG, LENG_SHORT, THICK, IMG_REGIST_TS, CLASS_NO, CLASS_NAME, ETC_OTC_NAME, ITEM_PERMIT_DATE, FORM_CODE_NAME, MARK_CODE_FRONT_ANAL, MARK_CODE_BACK_ANAL, MARK_CODE_FRONT_IMG, MARK_CODE_BACK_IMG, CHANGE_DATE, MARK_CODE_FRONT, MARK_CODE_BACK, ITEM_ENG_NAME, EDI_CODE) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
...
const calculateTotalPages = (totalCount) => {
    // 페이지당 보여질 개수 100 Row로 Divide
    return Math.ceil(totalCount / 100)
}
